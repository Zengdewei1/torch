...
transform_train = torch.transforms.Compose([
	torch.transforms.RandomCrop(32,padding=4),
	torch.transforms.RandomHorizontalFlip(),
	torch.transforms.ToTensor(),
	torch.transforms.Normalize(config.mean_cifar100,config.std_cifar100)]
	)
transform_test = torch.transforms.Compose([
	torch.transforms.ToTensor(),
	torch.transforms.Normalize(config.mean_cifar100,config.std_cifar100)]
	)
...
test_data = torchvision.datasets.CIFAR100(root='/home/lianfei/data/CIFAR100',train=False,download=True,transforms=transform_test)
...
if args.resume:
	...
	print("Building net...")
	net = Wide_ResNet(args.depth,args.widen_factor,args.dropout_rate,100)
...
def train(epoch):
	...
	correct = 0
	toral = 0
	optimizer = torch.optim.SGD(net.parameters(),lr=adjust_lr(args.lt,epoch),momentum=0.9,weight_decay=5e-4)
